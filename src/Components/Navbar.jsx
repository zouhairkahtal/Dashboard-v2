    ...
    <nav className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between h-16 bg-white/20 backdrop-blur-md rounded-2xl shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] border border-white/40 px-6">
        ...
        <div className="flex-shrink-0 flex items-center">
          <NavLink
            to="/"
            className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center gap-2 hover:scale-105 transition-transform duration-300"
          >
            <svg
        ...
        <div className="md:hidden flex items-center">
          ...
          >
            ...
            {!isOpen ? (
              ...
              <svg
                className="block h-6 w-6 transform rotate-90 transition-transform duration-200"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? "max-h-96 opacity-100 mt-3" : "max-h-0 opacity-0"}`}
      >
        <div className="px-3 pt-3 pb-4 space-y-2 sm:px-4 bg-white/60 backdrop-blur-xl rounded-2xl shadow-xl border border-white/50 relative z-50">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `block px-4 py-3 rounded-xl text-base font-semibold transition-colors duration-200 ${
                  isActive
